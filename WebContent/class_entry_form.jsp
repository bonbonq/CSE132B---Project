...
try
{
	...
	if (jdbc != null && jdbc.equals("addsection"))
	{
		...
		ps2 = conn.prepareStatement("SELECT * FROM department_course, course_coursenumber, coursenumber" + 
				" WHERE department_course.iddepartment = " + dept + 
		...
		rs2.next();
		int courseid = rs2.getInt("department_course.idcourse");
		String quarter = request.getParameter("quarter");
		ps3 = conn.prepareStatement("SELECT * FROM quarter WHERE year = " + year + "AND season = " + quarter + ")");
	...
	else if (todo == null)
	{
		...
		{	
			...
			<%
		}%></select><input type="hidden" name="todo" value="course"><input type="submit"></form>
		<%
	}
	...
	else if (todo.equals("course"))
	{
		...
		ps1 = conn.prepareStatement("SELECT number FROM coursenumber");
		rs1 = ps1.executeQuery();
		%><form action="class_entry_form.jsp">
		...
		<input type="radio" name="quarter" value="spring">Summer
		<select name="year">
